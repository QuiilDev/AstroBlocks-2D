...
pygame.display.set_caption("AstroBlocks 2D: Caves & Fluids Update")
...
def generate_textures():
    for block_id, color in BLOCK_COLORS.items():
        ...
        if block_id == GRASS:
            ...
            pygame.draw.rect(surf, (180, 40, 0), (4, 24, 8, 4))
            ...
            surf.fill((25, 20, 42))
            pygame.draw.rect(surf, (15, 10, 30), (0, 0, BLOCK_SIZE, BLOCK_SIZE), 2)
            pygame.draw.line(surf, (60, 40, 90), (4, 4), (16, 16), 2)
            pygame.draw.line(surf, (60, 40, 90), (24, 24), (36, 36), 1)
...
def save_world_to_file(slot):
    ...
    try:
        with open(filename, "w") as f:
            f.write(f"{player_x},{player_y},{jetpack_fuel},{current_dimension},{current_world_mode}\n")
...
def load_world_from_file(slot):
    global player_x, player_y, jetpack_fuel, current_dimension, current_world_mode, spawned_trees, wheat_growth_timer, earth_map, moon_map, world_map, player_hp
    filename = get_world_filename(slot)
...
def generate_new_world(mode):
    global earth_map, moon_map, world_map, spawned_trees, wheat_growth_timer, current_dimension, player_x, player_y, jetpack_fuel, current_world_mode, player_hp
    ...
    # --- КЛЕТОЧНЫЙ АВТОМАТ ДЛЯ ПЕЩЕР (ЗЕМЛЯ) ---
    ...
    for r in range(16, ROWS):
        for c in range(COLS):
            if cave_mask[r][c] == 1 and earth_map[r][c] != PORTAL and earth_map[r][c] != BOSS_ALTAR and earth_map[r][c] != LAVA:
...
creative_items = [
    ...
    WATER, OBSIDIAN, DIRT_WALL, STONE_WALL
]
...
while True:
    ...
    if current_game_state == STATE_PLAYING:
        ...
        # --- DYNAMIC FLUID PHYSICS (WATER & LAVA & OBSIDIAN) ---
                ...
                for row in range(ROWS - 2, -1, -1):
                    for col in range(COLS):
                        ...
                        if block in [WATER, LAVA]:
                            ...
                            if down in [AIR, DIRT_WALL, STONE_WALL]:
                                world_map[row + 1][col] = block
                                world_map[row][col] = AIR
                            else:
                                sides = []
                                if col > 0 and world_map[row][col - 1] in [AIR, DIRT_WALL, STONE_WALL]: sides.append(col - 1)
                                if col < COLS - 1 and world_map[row][col + 1] in [AIR, DIRT_WALL, STONE_WALL]: sides.append(col + 1)
                                if sides:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            if current_game_state == STATE_PLAYING:
                save_world_to_file(selected_world_slot)
        ...
        if event.type == pygame.KEYDOWN:
            if current_game_state == STATE_PLAYING:
                ...
                if event.key == pygame.K_i and not show_pause_menu and not show_chest:
                    if current_world_mode == MODE_CREATIVE:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if current_game_state == STATE_MAIN_MENU:
                ...
                if btn_play.collidepoint(mouse_x, mouse_y):
                    current_game_state = STATE_WORLD_SELECT
                    
            elif current_game_state == STATE_WORLD_SELECT:
                btn_mode_toggle = pygame.Rect(SCREEN_WIDTH // 2 - 150, 100, 300, 40)
                if btn_mode_toggle.collidepoint(mouse_x, mouse_y):
                    selected_creation_mode = MODE_SURVIVAL if selected_creation_mode == MODE_CREATIVE else MODE_CREATIVE
                ...
                for slot in range(1, 4):
                    ...
                    if btn_slot.collidepoint(mouse_x, mouse_y):
                        ...
                        if not load_world_from_file(slot):
                            generate_new_world(selected_creation_mode)
                            save_world_to_file(slot)
                ...
                if show_pause_menu:
                    ...
                    for idx, item in enumerate(creative_items):
                        ...
                        if pygame.Rect(inv_x + 20 + col_idx * 43, inv_y + 50 + row_idx * 43, 38, 38).collidepoint(mouse_x, mouse_y):
                    ...
                    if 0 <= block_col < COLS and 0 <= block_row < ROWS:
                        if event.button == 1:
                            if current_hand == BOW:
                                ...
                                # Замена на фоновые стены при уничтожении блоков недр
                                if block_row > 15:
                                    if world_map[block_row][block_col] in [STONE, COAL_ORE, IRON_ORE, GOLD_ORE, DIAMOND_ORE, REDSTONE_ORE, COPPER, BLACKSTONE]:
                                        world_map[block_row][block_col] = STONE_WALL
                                    elif world_map[block_row][block_col] == DIRT:
                                        world_map[block_row][block_col] = DIRT_WALL
                                    else:
                            ...
                            elif current_hand not in [PICKAXE, MINECART_ITEM, HOE, WHEAT_SEEDS] and world_map[block_row][block_col] in [AIR, DIRT_WALL, STONE_WALL]:
                                if current_world_mode == MODE_CREATIVE:
    ...
    if current_game_state == STATE_PLAYING and not show_pause_menu:
        ...
        is_in_water = False
        if 0 <= p_center_row < ROWS and 0 <= p_center_col < COLS:
            if world_map[p_center_row][p_center_col] == WATER:
                is_in_water = True

        if is_in_water:
            gravity = 0.08
            jump_force = -3.5
            current_speed = player_speed * 0.6
        else:
        ...
        if not show_inventory and not show_chest:
            ...
            if keys[pygame.K_SPACE] and current_hand == JETPACK and jetpack_fuel > 0:
                vel_y = -3.5 if is_in_water else -6.0
                is_grounded = False; jetpack_fuel -= 0.4
            ...
            if 0 <= foot_row < ROWS and 0 <= center_col < COLS:
                if world_map[foot_row][center_col] not in [AIR, WATER, DIRT_WALL, STONE_WALL]:
        ...
        for p in boss_projectiles:
            p["x"] += p["vx"]
            p_rect = pygame.Rect(p["x"], p["y"], 8, 8)
            p_hit = False
        ...
        if current_dimension == DIMENSION_EARTH and current_difficulty == DIFFICULTY_HARD and phase >= 0.75:
            ...
            if zombie_spawn_timer >= 180 and len(zombies) < 5:
                ...
                if 0 < spawn_x < WORLD_PIXEL_WIDTH:
                    ...
                    for r in range(ROWS):
                        if world_map[r][z_col] not in [AIR, WATER, DIRT_WALL, STONE_WALL]: z_row = r - 1; break
        ...
        for z in zombies:
            ...
            if 0 <= z_fr < ROWS and 0 <= z_cc < COLS and world_map[z_fr][z_cc] not in [AIR, WATER, DIRT_WALL, STONE_WALL]: z["y"] = z_fr * BLOCK_SIZE - player_h; z["vel_y"] = 0
        ...
        if current_world_mode == MODE_SURVIVAL and player_hp <= 0:
            ...
            current_dimension = DIMENSION_EARTH
            world_map = earth_map
            zombies.clear(); lunar_droids.clear(); lunar_lasers.clear()
    if current_game_state == STATE_PLAYING:
            ...
            for col in range(start_col, end_col):
                if 0 <= row < ROWS and 0 <= col < COLS:
                    ...
                    if block != AIR:
                        ...
                        # Динамическая пиксельная анимация пламени для текстуры факела
                        if block == TORCH:
                            fx = rect.x + 20
                            fy = rect.y + 14
                            # Огонь факела колышется за счёт смены фаз таймера
                        ...
                        # Обводка граней блоков (Autotiling) для создания бесшовного мира
                        if block not in [WATER, DIRT_WALL, STONE_WALL, TORCH]:
        ...
        # Маска мягкого факельного освещения в темноте пещер
        if night_darkness > 0 or current_dimension == DIMENSION_MOON:
        ...
        btn_mode_toggle = pygame.Rect(SCREEN_WIDTH // 2 - 150, 100, 300, 40)
        pygame.draw.rect(screen, (60, 60, 90), btn_mode_toggle)
        pygame.draw.rect(screen, (255, 255, 255), btn_mode_toggle, 2)
